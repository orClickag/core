...
test('related component references round-trip as a slug', () => {
  const source = `{% component component="header" /%}`;
  const document = fromMarkdoc(source);

  const node = document.get().doc.firstChild!;
  expect(node.type.name).toBe('component');
  expect(node.attrs.props.value.component).toBe('header');
  expect(toMarkdoc(document)).toBe(`${source}\n`);
});
