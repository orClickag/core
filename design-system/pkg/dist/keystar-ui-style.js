...
const BreakpointContext = /*#__PURE__*/React.createContext(['mobile']);
...
function useMatchedBreakpoints() {
  ...
  let [breakpoint, setBreakpoint] = useState(() => supportsMatchMedia ? getMatchedBreakpoints() : ['mobile']);
  useLayoutEffect(() => {
    ...
    const onResize = () => {
      ...
      setBreakpoint(prevMatched => {
        if (prevMatched.length !== matched.length || prevMatched.some((breakpoint, idx) => breakpoint !== matched[idx])) {
          return matched;
        }
        return prevMatched;
      });
