...
import type { cx } from "./emotion/index.js";
export type CSSProp = keyof CSSProperties;
export type Primitive = number | string | boolean | null | undefined;
export type Breakpoint = 'mobile' | 'tablet' | 'desktop' | 'wide';
export type StyleTarget = MaybeArray<CSSProp>;
export type StyleResolver<T = unknown> = (value: T) => T | number | string | boolean | null | undefined;
...
type KeysStartingWith<Keys, Prefix extends string> = Keys extends `${Prefix}${string}` ? Keys : never;
type BackgroundAliases = KeysStartingWith<ColorAliases, 'color.alias.background'>;
...
export type ColorBackground = keyof VoussoirTheme['color']['background'] | BackgroundAliases;
export type ColorBorder = keyof VoussoirTheme['color']['border'] | BorderAliases;
export type ColorForeground = keyof VoussoirTheme['color']['foreground'] | ColorAliases;
...
export type DimensionKey = `alias.${keyof Sizes['alias']}` | `border.${keyof Sizes['border']}` | `container.${keyof Sizes['container']}` | `element.${keyof Sizes['element']}` | `icon.${keyof Sizes['icon']}` | `scale.${keyof Sizes['scale']}`;
export type Loose<T> = T | number | (string & {});
export type LooseSizeDimension = DimensionKey | 0 | 'auto' | 'inherit' | '100%' | '100vh' | '100vw';
...
export type BaseStyleProps = RootStyleProps & {
    ...
    justifySelf?: Responsive<Exclude<CSSProperties['justifySelf'], 'left' | 'right'>>;
...
export type BoxAlignmentStyleProps = {
     ...
     * @default 'stretch'
     */
    justifyContent?: Responsive<Exclude<CSSProperties['justifyContent'], 'left' | 'right'>>;
    /**
...
} & BoxAlignmentStyleProps & Omit<BoxStyleProps, 'display'>;
export type GridStyleProps = {
    ...
    justifyItems?: Responsive<Exclude<CSSProperties['justifyItems'], 'left' | 'right'>>;
} & BoxAlignmentStyleProps & Omit<BoxStyleProps, 'display'>;
export {};
